...
const loopBlock = (symbols: string, attrs: string, edge: string = ''): string => {
    let str = '';
    if (edge === '->') {
        str = `
    {{~mod.${symbols} :_symbol}}
...
const importNodeAttrs =
    '{{?_symbol.isInternal}}shape="folder", color="#33a02c"{{??}}shape="folder", color="#e31a1c"{{?}}';
const importEdgeAttrs =
    '{{?_symbol.isInternal}}lhead="cluster_{{=mod.name}}", ltail="cluster_{{=mod.name}}_imports", color="#33a02c"{{??}}lhead="cluster_{{=mod.name}}", ltail="cluster_{{=mod.name}}_imports", color="#e31a1c"{{?}}';
...
node [fillcolor=2];
    ...
    subgraph cluster_{{=mod.name}}_imports {
      ...
      ${loopBlock('imports', importNodeAttrs)}
    }

    ${loopBlock('imports', importEdgeAttrs, '->')}
